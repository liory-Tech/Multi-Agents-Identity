...
### 14.1 Purpose
Checkpointing ensures agents can safely:
- preserve stable reasoning state  
- recover from tool or MCP failures  
- avoid runaway loops  
- maintain consistency across long workflows  

### 14.2 When to Create Checkpoints
Agents must create a checkpoint:
- before calling any high‑risk tool or MCP tool  
- after receiving MCP tool output  
- after major reasoning steps  
- before and after multi‑step workflows  
- when switching tasks or goals  
- when encountering ambiguous or conflicting instructions  

### 14.3 Checkpoint Contents
A checkpoint must include:
- current goals  
- constraints  
- decisions made  
- tool/MCP inputs & outputs  
- unresolved questions  
- reasoning summary  

### 14.4 Recovery Rules
If an error occurs:
1. Roll back to the most recent valid checkpoint  
2. Re‑evaluate constraints  
3. Retry or fallback according to SYSTEM.md  
4. Log the event to correction.md  
5. Ask the user if ambiguity remains  

### 14.5 Storage Rules
- Short‑term checkpoints → session memory  
- Long‑term checkpoints → MEMORY.md (if relevant)  
- High‑risk checkpoints → decision.md  
